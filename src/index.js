...
let genBank = {
  ...
  advicetwo: [
    ...
    ` by looking at developing ${focus}.`,
...
function shifter(skill) {
  if (skill === "reading") {
    return resources.reading[
      Math.floor(Math.random() * resources.reading.length)
    ];
  } else if (skill === "writing") {
    return resources.writing[
      Math.floor(Math.random() * resources.writing.length)
    ];
  } else if (skill === "listening") {
    return resources.listening[
      Math.floor(Math.random() * resources.listening.length)
    ];
  } else {
    return resources.speaking[
      Math.floor(Math.random() * resources.speaking.length)
    ];
  }
}
...
shifter("reading");
document.getElementById("app").innerHTML = compiler() + shifter("reading");
